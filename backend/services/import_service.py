...
def _rdflib_term_to_oxigraph(term):
    ...
    if isinstance(term, rdflib.Literal):
        ...
        return RDFLiteral(str(term), datatype=datatype, language=term.language)
...
async def parse_file(file_content: bytes, format: str) -> list[Triple]:
    """rdflib로 파일 파싱 → Triple 목록."""
    g = Graph()
    g.parse(data=file_content, format=format)
    triples = _graph_to_triples(g)
    logger.info("Parsed %d triples from file (format=%s)", len(triples), format)
    return triples
