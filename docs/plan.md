...
## Section 32 — Reasoner 개선

**날짜:** 2026-04-10

### 배경 및 문제점 분석

현재 `backend/services/reasoner_service.py` (463줄) + `backend/api/reasoner.py` (54줄) 구현 상태:

**[HIGH] H1 — Subgraph 선택 미작동**  
`ReasonerRunRequest.subgraph_entity_iris` 필드가 수신되나 `_execute()`에서 완전히 무시됨.  
`kg_iri = f"{ont_iri}/kg"` — 단일 Named Graph 하드코딩, 다른 Named Graph 전혀 참조 안 함.

**[HIGH] H2 — Named Graph 통합 미작동**  
Store에 저장된 Named Graph 목록을 조회하지 않고 `/kg` 하나만 사용.  
다중 Named Graph가 있는 온톨로지는 일부 트리플만 추론에 사용됨.

**[HIGH] H3 — Job Store 인메모리 (재시작 시 유실)**  
`_job_store: dict[str, dict] = {}` — 서버 재시작 시 모든 job 상태 유실.  
프론트엔드에서 진행 중 새로고침 시 job_id로 조회 불가.

**[MEDIUM] M1 — `reasoner_profile` 미연결**  
프론트엔드 타입에 `reasoner_profile: 'OWL_DL' | 'OWL_EL' | 'OWL_RL' | 'OWL_QL'` 있으나,  
백엔드 `ReasonerRunRequest`에 해당 필드 없음. HermiT는 항상 OWL DL 모드로만 실행됨.

**[MEDIUM] M2 — `infer_property_values` 비활성화 (owlready2 버그)**  
`owlready2.sync_reasoner_hermit(infer_property_values=False)` — TypeError silently caught.  
TransitiveProperty, InverseProperty 추론이 수행되지 않을 수 있음.

**[MEDIUM] M3 — OWL 2 위반 규칙 미완성**  
현재 SPARQL 위반 탐지: Cardinality, DomainRange, Disjoint 3가지만.  
누락: `owl:FunctionalProperty` 중복, `owl:minCardinality`, `owl:inverseOf` 비일관성.

**[LOW] L1 — 대용량 온톨로지 성능**  
`export_turtle(graph_iri)` — 전체 그래프를 메모리 버퍼에 로드 후 문자열 반환.  
수만 트리플 이상 시 메모리 spike 및 owlready2 파싱 지연.

**[LOW] L2 — 테스트 커버리지 부족**  
`reasoner_service.py`에 대한 단위 테스트 없음. Job 생명주기, 위반 탐지 로직 미검증.

**[LOW] L3 — 진단 로그 과다**  
`_detect_disjoint_violations`에서 `logger.info`로 전체 데이터 출력.  
프로덕션 환경에서 노이즈 과다.
...
#### H1 — `subgraph_entity_iris` 실제 적용

**파일:** `backend/services/reasoner_service.py`

1. `_execute()` 내부에서 `subgraph_entity_iris`가 non-empty일 때:
   - SPARQL CONSTRUCT로 해당 엔티티 + 직접 연결 트리플만 추출:
     ```sparql
     CONSTRUCT { ?s ?p ?o }
     WHERE {
       GRAPH ?g { ?s ?p ?o }
       FILTER(?s IN (<iri1>, <iri2>, ...) || ?o IN (<iri1>, <iri2>, ...))
     }
     ```
   - 추출 결과를 owlready2 World에 임시 그래프로 로드
2. `subgraph_entity_iris`가 비어 있으면 기존 full-graph 방식 유지

**검증:** 특정 엔티티 2개만 선택해 Reasoner 실행 시 해당 엔티티 관련 위반만 리포트

#### H2 — Named Graph 다중 통합

**파일:** `backend/services/reasoner_service.py`

1. `_execute()` 시작 시 `store.sparql_select`로 온톨로지 소속 Named Graph 목록 조회:
   ```sparql
   ...
   ```
2. 조회된 Named Graph 목록에서 CONSTRUCT 쿼리로 통합 트리플 생성
3. `kg_iri = f"{ont_iri}/kg"` 하드코딩 제거

**검증:** 2개 이상 Named Graph가 있는 온톨로지에서 모든 그래프의 트리플이 추론에 포함됨

#### H3 — Job 영속성 (SQLite)

**파일:** `backend/services/reasoner_service.py`, `backend/services/job_store.py` (신규)

1. `job_store.py` 신규 작성:
   - `JobStore` 클래스 — SQLite (`jobs.db`) 기반 CRUD
   - `create(job_id, ont_id)`, `update(job_id, status, result)`, `get(job_id)` 메서드
   - TTL: 7일 이상 된 completed/failed job 자동 삭제
2. `ReasonerService._job_store` → `JobStore` 인스턴스로 교체
3. `api/reasoner.py`에 `GET /jobs` 리스트 엔드포인트 추가

**검증:** 서버 재시작 후 이전 job_id로 조회 가능, 7일 초과 job은 자동 정리

#### M1 — `reasoner_profile` 백엔드 연결

**파일:** `backend/models/reasoner.py`, `backend/services/reasoner_service.py`

1. `ReasonerRunRequest`에 `reasoner_profile: Literal['OWL_DL', 'OWL_EL', 'OWL_RL', 'OWL_QL'] = 'OWL_DL'` 추가
2. `_run_hermit()` 내부:
   - `OWL_DL`: HermiT (기존)
   - `OWL_EL`: Pellet 또는 owlready2 EL subset (TransitiveProperty only)
   - `OWL_RL/QL`: SPARQL rule-based reasoning (owlready2 없이)
3. 프론트엔드 Reasoner UI의 profile 선택이 실제 추론 엔진에 반영됨

**검증:** OWL_EL 선택 시 HermiT 대신 경량 추론기 사용, 처리 속도 향상 확인

#### M2 — `infer_property_values` 복구 시도

**파일:** `backend/services/reasoner_service.py`

1. owlready2 버전 확인 (`pip show owlready2`)
2. `infer_property_values=True`가 가능한지 owlready2 최신 버전에서 테스트
3. 불가능하면 SPARQL로 TransitiveProperty/InverseOf 직접 추론 규칙 추가:
   ```sparql
   INSERT { GRAPH <inferred> { ?a ?p ?c } }
   WHERE { ?a ?p ?b . ?b ?p ?c . ?p a owl:TransitiveProperty }
   ```
4. 결과를 `ont_iri/inferred` Named Graph에 저장 (기존 방식 유지)

**검증:** TransitiveProperty가 정의된 온톨로지에서 추이적 관계가 inferred 그래프에 추가됨

#### M3 — 추가 위반 탐지 규칙

**파일:** `backend/services/reasoner_service.py`

추가할 SPARQL 기반 위반 탐지 (기존 3개 + 3개):

| 규칙 | SPARQL 패턴 | 에러 코드 |
|------|-------------|-----------|
| FunctionalProperty 중복 | `?p a owl:FunctionalProperty. ?s ?p ?o1, ?o2. FILTER(?o1 != ?o2)` | `FUNCTIONAL_VIOLATION` |
| minCardinality 미충족 | `?cls owl:minCardinality ?n. COUNT(?v) < ?n` | `MIN_CARDINALITY_VIOLATION` |
| inverseOf 비일관성 | `?p owl:inverseOf ?q. ?a ?p ?b. NOT EXISTS { ?b ?q ?a }` | `INVERSE_OF_VIOLATION` |
...
#### HIGH
- [x] **C32-H1** `subgraph_entity_iris` 결과 필터링 구현 (`_filter_by_entities` static method)
- [x] **C32-H2** Named Graph 다중 통합 (`_get_ont_graphs` + `_build_combined_rdfxml`, `kg_iri` 하드코딩 제거)
